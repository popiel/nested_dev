#!/usr/bin/env bash
# build-iso.sh — Build Ubuntu Server 26.04 autoinstall ISO for Dev VM
...
WORK_DIR="${OUTPUT_DIR}/dev-build-work"
REF="host_os_v0.1"
...
cp "${SCRIPT_DIR}/dev-firstboot.sh" "${EXTRACT_DIR}/dev-firstboot.sh"
chmod +x "${EXTRACT_DIR}/dev-firstboot.sh"
cp "${REPO_ROOT}/provision/personalization.sh" "${EXTRACT_DIR}/personalization.sh"
chmod +x "${EXTRACT_DIR}/personalization.sh"
# Copy Dockerfiles for lazy image builds
cp -r "${SCRIPT_DIR}/docker" "${EXTRACT_DIR}/docker"
...
log "Autoinstall ISO built: ${DEV_AUTO_ISO}"
log "SHA256: $(sha256sum "$DEV_AUTO_ISO" | awk '{print $1}')"

# --- 7. Record manifest ---
MANIFEST="${OUTPUT_DIR}/MANIFEST"
cat >> "$MANIFEST" <<EOF

## Dev ISO build — $(date -Is)
REF: ${REF}
Ubuntu ISO: ${UBUNTU_ISO_NAME}
Ubuntu ISO SHA256: $(sha256sum "$UBUNTU_ISO_PATH" | awk '{print $1}')
Auto ISO: $(basename "$DEV_AUTO_ISO")
Auto ISO SHA256: $(sha256sum "$DEV_AUTO_ISO" | awk '{print $1}')
EOF

log "Manifest updated: ${MANIFEST}"
log "=== Build complete ==="
log "Write ${DEV_AUTO_ISO} to USB or serve over HTTP for unattended install."
log "The VM will fetch dev-firstboot.sh from GitHub at REF ${REF} on first boot."
