...
- A simple HTTP file server (any) is available to serve `answer-host.toml` and
  provisioning payloads during install and first boot. If absent, `--answer-file`
  and embedded fragments can be used (see 6.x).
...
| File | Description |
|---|---|
| `iso/proxmox-ve_8.4-1.iso` | Official, unmodified PVE ISO (pin exact version `VERSION` in `build-iso.sh`) |
| `answer-host.toml` | Installer answer file (Section 4) |
| `provision/provision-host.sh` | First-boot provisioner entry point (Section 5) |
| `provision/frag/*.sh` | Provisioner fragments (GPU config, guest creation) |
| `provision/payloads/` | Uplift data: SSH keys, LAN IP plan, guest disk images (optional) |
...
[late-commands]
    "fetch-provisioner" = "wget -q -O /target/root/provision-host.sh http://192.168.1.2/provision/host/provision-host.sh && chmod +x /target/root/provision-host.sh"
...
[Service]
Type=oneshot
ExecStart=/root/provision-host.sh
[Install]
...
- `late-commands` run inside the installer's chroot (`/target/...` paths are the
  installed system). Use `wget` there since network is up at that point.
...
Fragments run in this order; each must be idempotent and log to
`/var/log/pve-firstboot.log`. The unit disables itself on success
...
- **vm 100 desktop**:
  ...
  - RDP published: prefer L2 routing so guests are routable on `vmbr0`; for NAT,
    ...
    `iptables` DNAT) mapping `:3389` to guest. Specify a defined plumb: the
    docroot carries a `netplan`/iptables fragment applied by the provisioner.
- **vm 101 llm**:
...
proxmox-auto-install-assistant verify build/host/answer-host.toml

# 2) prepare a bootable autoinstall ISO (network-fetched variant)
proxmox-auto-install-assistant prepare-iso \
    build/host/iso/proxmox-ve_8.4-1.iso \
    --fetch-from=http://<docroot-host>/host/answer-host.toml \
    --rng-source=autorandom
# output: proxmox-ve_8.4-1_auto.iso
```

Variants:
- Offline/embedded: `--answer-file=answer-host.toml` (no HTTP dependency).
- PXE: `proxmox-auto-install-assistant extract-dir | build-iso` per the installed
  tool's help; serve the docroot over HTTP + iPXE netboot, same answer file.
...
| `prepare-iso` `--fetch-from` unavailable offline | Use `--answer-file`; move provisioner payload onto the docroot kept on the LAN |