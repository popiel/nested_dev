#!/usr/bin/env bash
# build-iso.sh — Build Ubuntu Desktop 26.04 autoinstall ISO for desktop VM
# Downloads Ubuntu Desktop ISO, injects user-data, produces autoinstall ISO.
# REF: host_os_v0.1
#
...
UBUNTU_ISO_URL="${UBUNTU_BASE_URL}/${UBUNTU_DESKTOP_ISO}"
UBUNTU_ISO_NAME="${UBUNTU_DESKTOP_ISO}"

# Profile: i3 (default) or gnome
DESKTOP_PROFILE="${DESKTOP_PROFILE:-i3}"

log() { printf '[desktop-iso] %s\n' "$*"; }
die() { printf '[desktop-iso] ERROR: %s\n' "$*" >&2; exit 1; }
...
if [ ! -f "$UBUNTU_ISO_PATH" ]; then
    log "Downloading Ubuntu Desktop 26.04 ISO..."
...
# Append autoinstall parameters to kernel boot lines
...
# --- 6. Repackage ISO ---
DESKTOP_AUTO_ISO="${OUTPUT_DIR}/ubuntu-${UBUNTU_VERSION}-desktop-amd64_auto.iso"
log "Building autoinstall ISO..."
cd "$EXTRACT_DIR"
xorriso -as mkisofs \
    -o "$DESKTOP_AUTO_ISO" \
    -R -J -joliet-long \
    -V "UBUNTU-${UBUNTU_VERSION//./-}-DESKTOP-AUTO" \
...
log "Autoinstall ISO built: ${DESKTOP_AUTO_ISO}"
log "SHA256: $(sha256sum "$DESKTOP_AUTO_ISO" | awk '{print $1}')"

# --- 7. Record manifest ---
MANIFEST="${OUTPUT_DIR}/MANIFEST"
cat >> "$MANIFEST" <<EOF

## Desktop ISO build — $(date -Is)
REF: ${REF}
Profile: ${DESKTOP_PROFILE}
Ubuntu ISO: ${UBUNTU_ISO_NAME}
Ubuntu ISO SHA256: $(sha256sum "$UBUNTU_ISO_PATH" | awk '{print $1}')
Auto ISO: $(basename "$DESKTOP_AUTO_ISO")
Auto ISO SHA256: $(sha256sum "$DESKTOP_AUTO_ISO" | awk '{print $1}')
EOF

log "Manifest updated: ${MANIFEST}"
log "=== Build complete ==="
log "Write ${DESKTOP_AUTO_ISO} to USB or serve over HTTP for unattended install."
log "The VM will fetch first-boot.sh from GitHub at REF ${REF} on first boot."
